...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://olamicreas:mujeeb@localhost/user-messages'
...
def index():
    ...
    if request.method == "POST":
        ...
        if form.validate():
            ...
            return render_templates('users/new.html', form=form)
...
@app.route('/users/<int:user_id>/messgaes', methods=['GET', 'POST'])
...
def edit_messages(user_id, id):
    found_message = Message.query.get(id)
    return render_template('messages/edit.html', message=found_message)

@app.route('/users/<int:user_id>/messages/<int:id>/show', methods=['GET', 'POST', 'PATCH', 'DELETE'])
